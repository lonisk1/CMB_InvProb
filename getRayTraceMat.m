function [A] = getRayTraceMat(x_s, y_s, x_d, y_d, rad, T, E, n, n_E, n_g, visual)
%
% [A] = getRayTraceMat(x_s, y_s, x_d, y_d, rad, T, E, n, n_E, n_g, visual)
%
...
for j = 1:size(x_s,1) % loop over one dimension of source
  for k = 1:size(y_s,1) % loop over second dimension of source
    xs = x_s(j,1);
    ys = y_s(1,k);

    % Identify all detectors using circle with radius
    idx = find((x_d-xs).^2 + (y_d-ys).^2 <= (rad+0.5)^2 & (x_d-xs).^2 + (y_d-ys).^2 >= (rad-0.5)^2);
    ...
    for u = 1:n_E % loop over event planes
      ...
      for i = 1:length(idx) % loop over detector pixels
        ...
        if abs(intx) <= n_g && abs(inty) <= n_g % check if there is an intersection with the ray
          ...
          if intx == intx_lo && inty == inty_lo
            ...
            % corresponding node
            a1 = 1;
            j1 = find(x_s == intx_lo & y_s == inty_lo);
          ...
          elseif intx == intx_lo %(or intx_up: it doesn't matter which)
            ...
            a2 = 1 - a1;
            j1 = find(x_s == intx_lo & y_s == inty_lo);
            j2 = find(x_s == intx_lo & y_s == inty_up);
          ...
          elseif inty == inty_lo %(or inty_up: it doesn't matter which)
            ...
            a2 = 1 - a1;
            j1 = find(x_s == intx_lo & y_s == inty_lo);
            j2 = find(x_s == intx_up & y_s == inty_lo);
            ...
            j1 = find(x_s == intx_lo & y_s == inty_lo);
            j2 = find(x_s == intx_lo & y_s == inty_up);
            j3 = find(x_s == intx_up & y_s == inty_lo);
            j4 = find(x_s == intx_up & y_s == inty_up);
      ...
      ii_row = [ii_row; ii];
      jj_row = [jj_row; (u-1)*length(x_d) + jj];
      ss_row = [ss_row; ss];
    end
    if ~isempty(ii_row)
      if visual
          A_horzLayer = sparse(ii_row, jj_row, ss_row, size(x_d,1)*size(x_d,2), n*n*n_E);
      else
          A_horzLayer = sparse(ii_row, jj_row, ss_row, length(idx), n*n*n_E);
      end
